...
/**
 * @file   check_file_reader.c
 * @author Juan A. Rubio <juan.rubio@aratelia.com>
 *
 * @brief  Tizonia OpenMAX IL - Binary File Reader unit tests
 *
 *
 */
...
#define TIZ_LOG_CATEGORY_NAME "tiz.file_reader.check"
#endif

char *pg_rmd_path;
pid_t g_rmd_pid;

#define COMPONENT_NAME "OMX.Aratelia.file_reader.binary"
                    ...
                    OMX_U32 nData1, OMX_U32 nData2, OMX_PTR pEventData)
{
  ...
  TIZ_LOG (TIZ_LOG_TRACE, "Component Event [%s]",
  ...
  while (i < port_def.nBufferCountActual)
    {
      ...
      if (p_hdrlst[i]->nFilledLen)
        {
          TIZ_LOG (TIZ_LOG_TRACE, "Writing [%d] bytes to file [%s]",
          ...
          TIZ_LOG (TIZ_LOG_TRACE, "Bytes read [%d]", bytes_read);
          p_hdrlst[i]->nFilledLen = 0;
        }
...
main (void)
{
  ...
  TIZ_LOG (TIZ_LOG_TRACE, "Tizonia OpenMAX IL - Binary File Reader unit tests");
