                            ...
                            void * ap_data)
{
  ...
  std::pair<clients_map_t::iterator,bool> rv
    ...
    (std::make_pair<std::vector<unsigned char>, client_data>
     (uuid_vec, client_data(ap_cname, uuid_vec, grp_id, grp_pri,
                            apf_waitend, apf_preempt, apf_preempt_end,
                            ap_data)));
...
void
tizrmproxy::unregister_client(const tizrm_t * ap_rm)
{
  ...
  TIZ_LOG(TIZ_PRIORITY_TRACE, "Unregistering "
            "client with uuid [%s]...",
            uuid_str);
  ...
  if (it != clients_.end())
    {
      ...
      int32_t rc = TIZRM_SUCCESS;
      TIZ_LOG(TIZ_PRIORITY_TRACE, "Relinquishing rm resources "
...
int32_t
tizrmproxy::acquire(const tizrm_t * ap_rm, const uint32_t &rid,
                    const uint32_t &quantity)
{
  return invokerm(&com::aratelia::tiz::tizrmif_proxy::acquire,
                  ap_rm, rid, quantity);
}

int32_t
tizrmproxy::release(const tizrm_t * ap_rm, const uint32_t &rid,
                    const uint32_t &quantity)
{
  return invokerm(&com::aratelia::tiz::tizrmif_proxy::release,
                  ap_rm, rid, quantity);
}

int32_t
tizrmproxy::wait(const tizrm_t * ap_rm, const uint32_t &rid,
                 const uint32_t &quantity)
{
  return invokerm(&com::aratelia::tiz::tizrmif_proxy::wait,
                  ap_rm, rid, quantity);
}

int32_t
tizrmproxy::cancel_wait(const tizrm_t * ap_rm, const uint32_t &rid,
                        const uint32_t &quantity)
{
  return invokerm(&com::aratelia::tiz::tizrmif_proxy::cancel_wait,
                  ap_rm, rid, quantity);
}

int32_t
tizrmproxy::relinquish_all(const tizrm_t * ap_rm)
{
  int32_t rc = TIZRM_SUCCESS;
  assert(ap_rm);
  const std::vector<unsigned char> *p_uuid_vec
  ...
  if (clients_.count(*p_uuid_vec))
  ...
  else
    {
      ...
      char uuid_str[128];
      tiz_uuid_str(&((*p_uuid_vec)[0]), uuid_str);
      TIZ_LOG(TIZ_PRIORITY_TRACE, "Could not find the client with uuid [%s]...",
                uuid_str);
    }
...
int32_t
tizrmproxy::preemption_conf(const tizrm_t * ap_rm, const uint32_t &rid,
                           const uint32_t &quantity)
{
  return invokerm(&com::aratelia::tiz::tizrmif_proxy::preemption_conf,
                  ap_rm, rid, quantity);
}
                          ...
                          const std::vector< uint8_t >& uuid)
{
  ...
  TIZ_LOG(TIZ_PRIORITY_TRACE, "wait_complete on uuid [%s]...",
            uuid_str);
                           ...
                           const std::vector< uint8_t >& uuid)
{
  ...
  TIZ_LOG(TIZ_PRIORITY_TRACE, "preemption_req on uuid [%s]...",
            uuid_str);
                                ...
                                const std::vector< uint8_t >& uuid)
{
  ...
  TIZ_LOG(TIZ_PRIORITY_TRACE, "preemption_complete on uuid [%s]...",
            uuid_str);
...
int32_t
tizrmproxy::invokerm(pmf_t a_pmf, const tizrm_t * ap_rm, const uint32_t &rid,
                     const uint32_t &quantity)
{
  int32_t rc = TIZRM_SUCCESS;
  assert(ap_rm);
  ...
  if (clients_.count(*p_uuid_vec))
    {
      client_data &clnt = clients_[*p_uuid_vec];
      rc = (this->*a_pmf)(rid, quantity, clnt.cname_, *p_uuid_vec, clnt.grp_id_,
                          clnt.pri_);
    }
  else
    {
