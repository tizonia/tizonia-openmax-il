...
check_tizrmproxy_find_proc (const char *name)
{
  ...
  if (!(dir = opendir ("/proc")))
    {
      TIZ_LOG (TIZ_LOG_TRACE, "Could not open /proc");
      return -1;
    }
              ...
              if (strstr (first, name))
                {
                  fclose (fp);
                  closedir (dir);
                  TIZ_LOG (TIZ_LOG_TRACE, "Found process [%s] --> PID [%d]",
                             name, lpid);
                  return (pid_t) lpid;
                }
            }
  ...
  TIZ_LOG (TIZ_LOG_TRACE, "Process [%s] NOT FOUND", name);
...
START_TEST (test_proxy_acquire_and_release)
{
  ...
  tizrm_proxy_callbacks_t cbacks;
  ...
  if ((pid = check_tizrmproxy_find_proc ("tizrmd"))
    ...
    {
      TIZ_LOG (TIZ_LOG_TRACE, "RM Process [PID %d] FOUND", pid);
    }
  ...
  if (pid)
    {
      ...
      TIZ_LOG (TIZ_LOG_TRACE, "tizrm_proxy_init");
      error =
        tizrm_proxy_init (&p_rm, COMPONENT1_NAME,
                          (const OMX_UUIDTYPE *) &uuid_omx, &primgmt, &cbacks,
                          NULL);
      fail_if (error != TIZRM_SUCCESS);

      /* error = tizrm_proxy_version(&p_rm); */
      error = tizrm_proxy_acquire (&p_rm, TIZRM_RESOURCE_DUMMY, 1);
      TIZ_LOG (TIZ_LOG_TRACE, "tizrm_proxy_acquire returned [%d]", error);
      fail_if (error != TIZRM_SUCCESS);

      error = tizrm_proxy_release (&p_rm, TIZRM_RESOURCE_DUMMY, 1);
      TIZ_LOG (TIZ_LOG_TRACE, "tizrm_proxy_release returned [%d]", error);
      fail_if (error != TIZRM_SUCCESS);

      error = tizrm_proxy_destroy (&p_rm);
      fail_if (error != TIZRM_SUCCESS);
      ...
      TIZ_LOG (TIZ_LOG_TRACE, "DB comparison check [%s]",
                 (rc == 0 ? "SUCCESS" : "FAILED"));
      fail_if (rc != 0);
...
START_TEST (test_proxy_wait_cancel_wait)
{
  ...
  if ((pid = check_tizrmproxy_find_proc ("tizrmd"))
      || (pid = check_tizrmproxy_find_proc ("lt-tizrmd")))
    {
      TIZ_LOG (TIZ_LOG_TRACE, "RM Process [PID %d] FOUND -- > SKIPPING THIS TEST", pid);
    }
  else
    {
      /* Init the RM database */
      ...
      if (pid)
        {
          ...
          error = tizrm_proxy_wait (&p_rm, TIZRM_RESOURCE_DUMMY, 1);
          TIZ_LOG (TIZ_LOG_TRACE, "tizrm_proxy_wait returned [%d]", error);
          fail_if (error != TIZRM_SUCCESS);

          /* Now cancel the wait */
          error = tizrm_proxy_cancel_wait (&p_rm, TIZRM_RESOURCE_DUMMY, 1);
          TIZ_LOG (TIZ_LOG_TRACE, "tizrm_proxy_cancel_wait returned [%d]",
                     error);
          fail_if (error != TIZRM_SUCCESS);

          error = tizrm_proxy_destroy (&p_rm);
          TIZ_LOG (TIZ_LOG_TRACE, "tizrm_proxy_destroy returned [%d]", error);
          fail_if (error != TIZRM_SUCCESS);
      ...
      if (pid)
        {
          ...
          TIZ_LOG (TIZ_LOG_TRACE, "tizrm_proxy_init : [%s]", COMPONENT1_NAME);
          error =
          ...
          TIZ_LOG (TIZ_LOG_TRACE, "tizrm_proxy_init : [%s]", COMPONENT2_NAME);
          error =
          ...
          TIZ_LOG (TIZ_LOG_TRACE, "tizrm_proxy_acquire returned (rm1) [%d]",
                     error);
          fail_if (error != TIZRM_SUCCESS);
          ...
          TIZ_LOG (TIZ_LOG_TRACE, "Wait complete (rm2)");

          /* Component2 releases the resource  */
          error = tizrm_proxy_release (&p_rm2, TIZRM_RESOURCE_DUMMY, 1);
          fail_if (error != TIZRM_SUCCESS);
          TIZ_LOG (TIZ_LOG_TRACE, "tizrm_proxy_release returned (rm2) [%d]",
                     error);

          /* Destroy the rm hdls */
          TIZ_LOG (TIZ_LOG_TRACE, "Destroying rm hdls");
          error = tizrm_proxy_destroy (&p_rm1);
          fail_if (error != TIZRM_SUCCESS);
          TIZ_LOG (TIZ_LOG_TRACE, "tizrm_proxy_destroy returned (rm1) [%d]",
                     error);

          error = tizrm_proxy_destroy (&p_rm2);
          fail_if (error != TIZRM_SUCCESS);
          TIZ_LOG (TIZ_LOG_TRACE, "tizrm_proxy_destroy returned (rm2) [%d]",
                     error);
      ...
      if (pid)
        {
          ...
          TIZ_LOG (TIZ_LOG_TRACE, "Preemption is in progress (rm1) [%d]",
                     error);
          ...
          TIZ_LOG (TIZ_LOG_TRACE, "Preemption request verified (rm1) [%d]",
                     error);
          ...
          TIZ_LOG (TIZ_LOG_TRACE, "Preemption completion verified (rm2) [%d]",
                     error);
