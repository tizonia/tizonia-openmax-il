...
static const char *TIZ_RM_DB_DROP_ALLOC_TABLE = "drop table if exists allocation";
...
tizrm_error_t
tizrmdb::acquire_resource(const unsigned int &rid, const unsigned int &quantity,
                          ...
                          const unsigned int &pri)
{
  ...
  snprintf(query, 500,
           "insert or replace into allocation (cname, uuid, grpid, pri, resid, allocation) "
           "values('%s', '%s', %d, %d, %d, %d)",
...
tizrm_error_t
tizrmdb::release_resource(const unsigned int &rid, const unsigned int &quantity,
                          const std::string &cname,
                          const std::vector< unsigned char > &uuid,
                          const unsigned int &grpid,
                          const unsigned int &pri)
