...
#define TIZ_LOG_CATEGORY_NAME "tiz.osal.http"
#endif
...
{
  TIZ_LOG (TIZ_LOG_TRACE, "*** on message begin ***");
  return 0;
}
...
on_url (http_parser * ap_parser, const char *ap_at, size_t a_length)
{
  ...
  TIZ_LOG (TIZ_LOG_TRACE, "*** on url [%.*s] ***", (int) a_length, ap_at);
...
{
  TIZ_LOG (TIZ_LOG_TRACE, "*** on status complete ***");
  return 0;
}
...
on_header_field (http_parser * ap_parser, const char *ap_at, size_t a_length)
{
  ...
  TIZ_LOG (TIZ_LOG_TRACE, "*** on header field [%.*s] ***", (int) a_length,
           ap_at);
...
on_header_value (http_parser * ap_parser, const char *ap_at, size_t a_length)
{
  ...
  TIZ_LOG (TIZ_LOG_TRACE, "*** on header value [%.*s] ***", (int) a_length,
           ap_at);
...
{
  TIZ_LOG (TIZ_LOG_TRACE, "*** on headers complete ***");
  return 0;
}
...
{
  TIZ_LOG (TIZ_LOG_TRACE, "*** on message complete ***");
  return 0;
}
                      ...
                      tiz_http_parser_type_t type)
{
  ...
  if (NULL == (p_hp = (tiz_http_parser_ptr_t) tiz_mem_calloc
    ...
    {
      TIZ_LOG (TIZ_LOG_ERROR, "Error allocating http parser structure.");
  ...
  if (NULL == (p_hp->p_dict = avl_new_avl_tree (compare_kv_pairs, NULL)))
    {
      TIZ_LOG (TIZ_LOG_ERROR, "Error allocating avl tree structure.");
      rc = OMX_ErrorInsufficientResources;
      goto end;
    }
