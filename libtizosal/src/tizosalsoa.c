 ...
 *
 * @brief  Tizonia OpenMAX IL OSAL - Small object allocation
 *
 *
 */
...
#define TIZ_LOG_CATEGORY_NAME "tiz.osal.soa"
#endif
...
static inline uint8_t *get_usr_ptr (slice_t *p_slice)
{
  return ((uint8_t *)p_slice + SLICE_PREAMBLE_SZ);
}

static inline slice_t *get_slice_ptr (void *p_usr)
{
...
/*@null@*/ static slice_t *alloc_chunk (tiz_soa_t *p_soa, int32_t chunk_class)
{
...
/*@null@*/ void *tiz_soa_calloc (tiz_soa_t *p_soa, size_t size)
{
...
void tiz_soa_free (tiz_soa_t *p_soa, void *p_addr)
{
  assert (p_soa != NULL);
