...
{
  tiz_srv_t *p_obj = super_ctor (tizsrv, ap_obj, app);
...
servant_dtor (void *ap_obj)
{
  ...
  return super_dtor (tizsrv, ap_obj);
}
                            ...
                            OMX_DIRTYPE dir, OMX_HANDLETYPE ap_tcomp)
{
  ...
  assert (NULL != p_obj->p_cbacks_);
  assert (NULL != p_obj->p_cbacks_->EventHandler);
  TIZ_LOGN (TIZ_PRIORITY_DEBUG, tiz_api_get_hdl (ap_obj),
  ...
  if (ap_tcomp)
    {
      if (OMX_DirInput == dir)
        {
          TIZ_LOGN (TIZ_PRIORITY_TRACE, tiz_api_get_hdl (ap_obj), "OMX_FillThisBuffer");
          (void) OMX_FillThisBuffer (ap_tcomp, p_hdr);
        }
      else
        {
          TIZ_LOGN (TIZ_PRIORITY_TRACE, tiz_api_get_hdl (ap_obj), "OMX_EmptyThisBuffer");
