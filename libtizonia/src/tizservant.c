...
static OMX_ERRORTYPE srv_tick (const void *ap_obj)
{
  ...
  /* Receive the message */
  rc = tiz_pqueue_receive (p_obj->p_pq_, &p_msg);
  ...
  /* Process the message */
  rc = tiz_srv_dispatch_msg (p_obj, p_msg);
  ...
  if (OMX_ErrorNone != rc && OMX_ErrorNoMore != rc)
    {
      tiz_srv_issue_err_event (p_obj, rc);
    }
                             ...
                             /*@null@*/ OMX_PTR ap_eventdata)
{
  ...
  (void)p_obj->p_cbacks_->EventHandler (handleOf (ap_obj), p_obj->p_appdata_,
  ...
  if (ap_tcomp)
  ...
  else
    {
      ...
      (OMX_HANDLETYPE, OMX_PTR, OMX_BUFFERHEADERTYPE *)
          = (dir == OMX_DirInput ? p_obj->p_cbacks_->EmptyBufferDone
                                 : p_obj->p_cbacks_->FillBufferDone);
...
static void *srv_soa_calloc (void *ap_obj, size_t a_size)
{
  tiz_srv_t *p_obj = ap_obj;
  assert (NULL != p_obj);
  return tiz_soa_calloc (p_obj->p_soa_, a_size);
}
...
static void srv_soa_free (void *ap_obj, void *ap_addr)
{
  tiz_srv_t *p_obj = ap_obj;
  assert (NULL != p_obj);
  tiz_soa_free (p_obj->p_soa_, ap_addr);
}
  ...
  while ((selector = va_arg (ap, voidf)))
    {
      ...
      if (selector == (voidf)tiz_srv_set_allocator)
        ...
        {
          *(voidf *)&p_obj->soa_calloc = method;
        }
      else if (selector == (voidf)tiz_srv_soa_free)
        {
          *(voidf *)&p_obj->soa_free = method;
        }
    }
  /*@end@*/
  /* NOTE: Stop ignoring splint warnings in this section  */
