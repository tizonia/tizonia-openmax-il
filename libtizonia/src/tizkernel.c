...
#define TIZ_KRN_INIT_MSG_OOM(obj,hdl,msg,msgtype)                       \
  do {                                                                  \
    tiz_ret_val_on_err ( (msg = init_kernel_message (obj, hdl, (msgtype))), \
...
static OMX_ERRORTYPE dispatch_port_disable (void *ap_obj,
                                            ...
                                            tiz_kernel_msg_sendcommand_t *
                                            ap_msg_sc);
static OMX_ERRORTYPE dispatch_port_enable (void *ap_obj, OMX_HANDLETYPE ap_hdl,
                                           tiz_kernel_msg_sendcommand_t *
                                           ap_msg_sc);
static OMX_ERRORTYPE dispatch_port_flush (void *ap_obj, OMX_HANDLETYPE ap_hdl,
                                          tiz_kernel_msg_sendcommand_t *
                                          ap_msg_sc);
static OMX_ERRORTYPE dispatch_mark_buffer (void *ap_obj, OMX_HANDLETYPE ap_hdl,
                                           tiz_kernel_msg_sendcommand_t *
                                           ap_msg_sc);
static OMX_ERRORTYPE flush_marks (void *ap_obj, OMX_PTR ap_port);

static OMX_ERRORTYPE check_pid (const tiz_kernel_t *ap_obj, OMX_U32 a_pid);
static inline tiz_vector_t * get_port (const tiz_kernel_t *ap_obj, OMX_U32 a_pid);
                               ...
                               OMX_PTR ap_data2)
{
  OMX_BOOL rc = OMX_FALSE;
  tiz_kernel_msg_t *p_msg = ap_elem;
  tiz_kernel_t *p_obj = ap_data2;

  assert (NULL != ap_elem);
  assert (NULL != ap_data2);

  if (p_msg->class == ETIZKernelMsgEmptyThisBuffer
      || p_msg->class == ETIZKernelMsgFillThisBuffer)
    {
      ...
      pid = p_msg->class == ETIZKernelMsgEmptyThisBuffer ?
...
{
  tiz_kernel_t *p_obj = (tiz_kernel_t *) ap_obj;
...
get_ingress_lst (const tiz_kernel_t *ap_obj, OMX_U32 a_pid)
{
...
static inline tiz_vector_t *
get_egress_lst (const tiz_kernel_t *ap_obj, OMX_U32 a_pid)
{
  tiz_vector_t * p_list = NULL;
  assert (NULL != ap_obj);
...
static inline tiz_vector_t *
get_port (const tiz_kernel_t *ap_obj, OMX_U32 a_pid)
{
...
add_to_buflst (void *ap_obj, tiz_vector_t * ap_dst2darr,
               const OMX_BUFFERHEADERTYPE * ap_hdr, const void *ap_port)
{
  const tiz_kernel_t *p_obj = ap_obj;
  tiz_vector_t *p_list = NULL;
...
static OMX_ERRORTYPE
check_pid (const tiz_kernel_t *ap_obj, OMX_U32 a_pid)
{
  assert (NULL != ap_obj);
  ...
  do
    {
      ...
      TIZ_LOGN (TIZ_TRACE, p_hdl, "pid [%d] loop index=[%d] egress length [%d] "
                "- p_thdl [%p]...", pid, i, tiz_vector_length (p_list), p_thdl);
...
{
  tiz_kernel_t *p_obj = super_ctor (tizkernel, ap_obj, app);
                    ...
                    OMX_PTR ap_cmd_data)
{
  ...
  TIZ_KRN_INIT_MSG_OOM (ap_obj, ap_hdl, p_msg, ETIZKernelMsgSendCommand);
                   ...
                   OMX_U32 a_pid, OMX_BUFFERHEADERTYPE * ap_hdr)
{
...

  {
    ...
    if (!buf_count && was_being_disabled)
      {
        complete_port_disable ((tiz_kernel_t *) p_obj, p_port,
                               a_pid, OMX_ErrorNone);
      }
  }
                        ...
                        OMX_HANDLETYPE ap_hdl, OMX_BUFFERHEADERTYPE * ap_hdr)
{
  ...
  TIZ_KRN_INIT_MSG_OOM (ap_obj, ap_hdl, p_msg, ETIZKernelMsgEmptyThisBuffer);
                       ...
                       OMX_HANDLETYPE ap_hdl, OMX_BUFFERHEADERTYPE * ap_hdr)
{
  ...
  TIZ_KRN_INIT_MSG_OOM (ap_obj, ap_hdl, p_msg, ETIZKernelMsgFillThisBuffer);
...
kernel_dispatch_msg (const void *ap_obj, OMX_PTR ap_msg)
{
  ...
  TIZ_LOGN (TIZ_TRACE, tiz_servant_get_hdl (p_obj), "Processing [%s]...",
  ...
  do
    {
      ...
      tiz_port_update_tunneled_status (p_port, tiz_servant_get_hdl (p_obj),
      ...
      if (TIZ_PORT_IS_ENABLED_TUNNELED_SUPPLIER_AND_NOT_POPULATED (p_port))
        {
          ...
          if (OMX_ErrorNone != (rc = tiz_port_populate (p_port,
            ...
            {
              TIZ_LOGN (TIZ_ERROR, tiz_servant_get_hdl (p_obj),
...
kernel_prepare_to_transfer (void *ap_obj, OMX_U32 a_pid)
{
  ...
  TIZ_LOGN (TIZ_TRACE, tiz_servant_get_hdl (p_obj), "pid [%d]", a_pid);
...
kernel_transfer_and_process (void *ap_obj, OMX_U32 a_pid)
{
  ...
  TIZ_LOGN (TIZ_TRACE, tiz_servant_get_hdl (p_obj), "T&P pid [%d]", a_pid);
...
kernel_stop_and_return (void *ap_obj)
{
  ...
  TIZ_LOGN (TIZ_TRACE, tiz_servant_get_hdl (p_obj), "stop and return...[%p]",
            ap_obj);
  ...
  for (i = 0; i < nports && OMX_ErrorNone == rc; ++i)
    {
      ...
      if (TIZ_PORT_GET_CLAIMED_COUNT (p_port) >= 0)
        {
          void *p_prc        = tiz_get_prc (tiz_servant_get_hdl (p_obj));
          if (OMX_ErrorNone
...
tiz_kernel_register_port (const void *ap_obj, OMX_PTR ap_port,
                         OMX_BOOL ais_config)
{
  const tiz_kernel_class_t *class = classOf (ap_obj);
...
{
  const tiz_kernel_class_t *superclass = super (a_class);
...
OMX_ERRORTYPE
kernel_find_managing_port (const tiz_kernel_t * ap_krn,
  ...
  if (OMX_ErrorNone == tiz_port_find_index (ap_krn->p_cport_, a_index))
    {
      *app_port = ap_krn->p_cport_;
      TIZ_LOGN (TIZ_TRACE, tiz_servant_get_hdl (ap_krn),
                     ...
                     OMX_U32 a_pos, OMX_BUFFERHEADERTYPE ** app_hdr)
{
  ...
  OMX_BUFFERHEADERTYPE *p_hdr = NULL;
  OMX_PTR p_port = NULL;
  OMX_HANDLETYPE hdl = tiz_servant_get_hdl (p_obj);
  OMX_DIRTYPE pdir = OMX_DirMax;
        ...
        if (TIZ_PORT_IS_TUNNELED (*pp_port) && TIZ_PORT_IS_SUPPLIER (*pp_port))
          {
            ...
            if (!TIZ_PORT_MAY_CALL_USE_BUFFER (*pp_port))
              {
                status &= ~(OMX_PORTSTATUS_ACCEPTUSEBUFFER);
                TIZ_LOGN (TIZ_DEBUG, tiz_servant_get_hdl (p_obj),
...
{
  tiz_kernel_class_t *p_obj = super_ctor (tizkernel_class, ap_obj, app);
  typedef void (*voidf) ();
  voidf selector;
  va_list ap;
  va_copy (ap, *app);
