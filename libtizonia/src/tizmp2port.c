...
mp2port_SetParameter_common (const void *ap_obj,
...
{
  tiz_mp2port_t *p_obj = super_ctor (typeOf (ap_obj, "tizmp2port"), ap_obj, app);
...
static OMX_ERRORTYPE
mp2port_GetParameter (const void *ap_obj,
                      OMX_HANDLETYPE ap_hdl,
                      OMX_INDEXTYPE a_index, OMX_PTR ap_struct)
{
    ...
    default:
      {
        if (OMX_TizoniaIndexParamAudioMp2 == a_index)
        ...
        else
          {
            ...
            rc = super_GetParameter (typeOf (ap_obj, "tizmp2port"),
...
static OMX_ERRORTYPE
mp2port_SetParameter (const void *ap_obj,
                      OMX_HANDLETYPE ap_hdl,
                      OMX_INDEXTYPE a_index, OMX_PTR ap_struct)
{
  tiz_mp2port_t *p_obj = (tiz_mp2port_t *) ap_obj;
        ...
        if (OMX_TizoniaIndexParamAudioMp2 == a_index)
          {
            ...
            if ((OMX_DirOutput == p_base->portdef_.eDir)
              ...
              {
                rc = mp2port_SetParameter_common (ap_obj, ap_hdl, a_index, ap_struct);
              }
          }
        else
          {
            /* Try the parent's indexes */
            rc = super_SetParameter (typeOf (ap_obj, "tizmp2port"),
                                     ap_obj, ap_hdl, a_index, ap_struct);
          }
      }
      break;
...
static OMX_ERRORTYPE
mp2port_SetParameter_internal (const void *ap_obj,
                               OMX_HANDLETYPE ap_hdl,
                               OMX_INDEXTYPE a_index, OMX_PTR ap_struct)
{
  tiz_mp2port_t *p_obj = (tiz_mp2port_t *) ap_obj;
...
tiz_mp2port_class_init (void * ap_tos, void * ap_hdl)
{
  ...
  void * tizmp2port_class = factory_new
    ...
    (classOf (tizaudioport), "tizmp2port_class", classOf (tizaudioport), sizeof (tiz_mp2port_class_t),
     /* TIZ_CLASS_COMMENT: */
     ap_tos, ap_hdl,
