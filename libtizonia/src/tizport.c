           ...
           OMX_U32 * ap_size, OMX_U8 ** app_buf, OMX_PTR * app_portPrivate)
{
  ...
  if (!(p_buf = p_obj->opts_.mem_hooks.pf_alloc
    ...
    {
      TIZ_ERROR (handleOf (p_obj), "[OMX_ErrorInsufficientResources] : "
                 "allocating the buffer on PORT [%d]", p_obj->pid_);
      return OMX_ErrorInsufficientResources;
    }

  TIZ_TRACE (handleOf (p_obj), "size [%u] pBuffer [%p]",
            *ap_size, p_buf);
                 ...
                 OMX_PTR /*@null@*/ ap_eglimage)
{
  ...
  p_bps->p_eglimage = ap_eglimage; /* NULL unless OMX_UseEGLImage is being used */
    ...
    default:
      {
        if (OMX_TizoniaIndexParamBufferPreAnnouncementsMode == a_index)
          ...
          {
            TIZ_ERROR (ap_hdl, "[OMX_ErrorUnsupportedIndex] - [%s] on PORT [%d]",
  ...
  if (OMX_ErrorNone != register_header (p_obj, p_hdr, OMX_FALSE, NULL))
    {
      tiz_mem_free (p_hdr);
      TIZ_ERROR (ap_hdl, "[OMX_ErrorInsufficientResources] : "
                "While registering the OMX header on PORT [%d]", p_obj->pid_);
      return OMX_ErrorInsufficientResources;
    }
...
static OMX_ERRORTYPE
port_UseEGLImage (const void *ap_obj,
                  OMX_HANDLETYPE ap_hdl,
                  OMX_BUFFERHEADERTYPE ** app_hdr,
                  OMX_U32 a_pid,
                  OMX_PTR ap_app_priv, OMX_U8 * ap_eglimage)
{
  ...
  if (!p_hdr)
    {
      TIZ_ERROR (ap_hdl, "[OMX_ErrorInsufficientResources] : "
                "While allocating the OMX header on PORT [%d]", p_obj->pid_);
      return OMX_ErrorInsufficientResources;
    }
  ...
  if (OMX_ErrorNone != register_header (p_obj, p_hdr, OMX_FALSE, ap_eglimage))
    {
      tiz_mem_free (p_hdr);
      TIZ_ERROR (ap_hdl, "[OMX_ErrorInsufficientResources] : "
                "While registering the OMX headeron PORT [%d]", p_obj->pid_);
      return OMX_ErrorInsufficientResources;
    }
                     ...
                     OMX_U32 a_pid, OMX_PTR ap_app_priv, OMX_U32 a_size)
{
  ...
  if (NULL == (p_hdr = tiz_mem_calloc (1, sizeof (OMX_BUFFERHEADERTYPE))))
    {
      TIZ_ERROR (ap_hdl, "[OMX_ErrorInsufficientResources] : "
                 "allocating the buffer header on PORT [%d]", p_obj->pid_);
      return OMX_ErrorInsufficientResources;
    }
...
port_populate (const void *ap_obj)
{
  ...
  TIZ_TRACE (handleOf (ap_obj), "PORT [%d]...",
            p_obj->pid_);
  ...
  if (OMX_ErrorNone
    ...
    {
      TIZ_ERROR (handleOf (ap_obj), "[%s] while retrieving the "
                "port definition from peer.. ", tiz_err_to_str (rc));
      return OMX_ErrorUndefined;
    }

  TIZ_TRACE (handleOf (ap_obj), "Retrieved port definition "
      ...
      if (p_obj->portdef_.nBufferCountActual < nbufs)
        {
          ...
          TIZ_TRACE (handleOf (ap_obj), "PORT [%d] own "
      ...
      else
        {
          ...
          TIZ_TRACE (handleOf (ap_obj), "Peer port index [%d] "
          ...
          if (OMX_ErrorNone
              != (rc = OMX_SetParameter (p_obj->thdl_,
                                         ...
                                         &port_def)))
            {
              TIZ_LOGN(TIZ_PRIORITY_ERROR, handleOf (ap_obj), "[%s] while updating "
      ...
      do
        {
          ...
          TIZ_DEBUG (handleOf (ap_obj), "OMX_UseBuffer "
  ...
  TIZ_TRACE (handleOf (ap_obj), "PORT [%d] populated",
            p_obj->pid_);
...
port_update_tunneled_status (void *ap_obj, OMX_U32 a_port_status)
{
  ...
  if (OMX_ErrorNone != rc)
    {
      TIZ_ERROR (handleOf (ap_obj), "WARNING: "
                                 ...
                                 OMX_U32 a_port_status_flag)
{
  ...
  TIZ_TRACE (handleOf (ap_obj), "peer nTunneledPortStatus = "
