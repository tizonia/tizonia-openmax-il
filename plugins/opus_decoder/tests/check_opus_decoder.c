...
/**
 * @file   check_opus_decoder.c
 * @author Juan A. Rubio <juan.rubio@aratelia.com>
 *
 * @brief  Tizonia OpenMAX IL - Opus Decoder unit tests
 *
 *
 */
...
#define TIZ_LOG_CATEGORY_NAME "tiz.opus_decoder.check"
#endif
...
static char OPUS_DEC_COMPONENT_NAME[] = "OMX.Aratelia.audio_decoder.opus";
...
#define OPUS_DECODER_TEST_TIMEOUT 360
...
static const char *pg_cnames[] = {
  OGG_DMUX_COMPONENT_NAME, OPUS_DEC_COMPONENT_NAME, PCM_RND_COMPONENT_NAME
};
...
init_test_data()
{
  ...
  p_testfile1 = tiz_rcfile_get_value("plugins-data",
                                     "OMX.Aratelia.audio_decoder.opus.testfile1_uri");

  p_testfile2 = tiz_rcfile_get_value("plugins-data",
                                     "OMX.Aratelia.audio_decoder.opus.testfile2_uri");
...
START_TEST (test_opus_playback)
{
  OMX_ERRORTYPE error = OMX_ErrorNone;
  OMX_HANDLETYPE p_oggdmux = NULL, p_opusdec = NULL, p_pcmrnd = NULL;
  ...
  /* --------------------------- */
  /* Instantiate the opus decoder */
  /* --------------------------- */
  error = OMX_GetHandle (&p_opusdec, OPUS_DEC_COMPONENT_NAME, (OMX_PTR *) (&dec_ctx),
                         &_check_cbacks);
  fail_if (OMX_ErrorNone != error);
  TIZ_LOG (TIZ_PRIORITY_TRACE, "[%s] hdl [%p]", OPUS_DEC_COMPONENT_NAME, p_opusdec);
  pg_hdls[1] = p_opusdec;
  ...
  error = OMX_GetParameter (p_opusdec, OMX_IndexParamPortDefinition, &dec_port_def0);
  fail_if (OMX_ErrorNone != error);

  TIZ_LOG (TIZ_PRIORITY_TRACE, "[%s] port #0 nBufferSize [%d]",
             OPUS_DEC_COMPONENT_NAME, dec_port_def0.nBufferSize);
  TIZ_LOG (TIZ_PRIORITY_TRACE, "[%s] port #0 nBufferCountActual [%d]",
             OPUS_DEC_COMPONENT_NAME, dec_port_def0.nBufferCountActual);
  ...
  error = OMX_GetParameter (p_opusdec, OMX_IndexParamPortDefinition, &dec_port_def1);
  fail_if (OMX_ErrorNone != error);

  TIZ_LOG (TIZ_PRIORITY_TRACE, "[%s] port #1 nBufferSize [%d]",
             OPUS_DEC_COMPONENT_NAME, dec_port_def1.nBufferSize);
  TIZ_LOG (TIZ_PRIORITY_TRACE, "[%s] port #1 nBufferCountActual [%d]",
             OPUS_DEC_COMPONENT_NAME, dec_port_def1.nBufferCountActual);
  ...
  error = OMX_GetParameter (p_opusdec, OMX_IndexParamAudioPcm, &dec_pcm_mode);
  fail_if (OMX_ErrorNone != error);
  ...
  error = OMX_SetParameter (p_opusdec, OMX_IndexParamCompBufferSupplier, &supplier);
  TIZ_LOG (TIZ_PRIORITY_TRACE, "[%s] OMX_BufferSupplyInput [%s]",
             OPUS_DEC_COMPONENT_NAME, tiz_err_to_str(error));
  fail_if (OMX_ErrorNone != error);
  ...
  /* --------------------------------- */
  error = OMX_SetupTunnel(p_oggdmux, 0, p_opusdec, 0);
  ...
  /* ----------------------------------- */
  error = OMX_SetupTunnel(p_opusdec, 1, p_pcmrnd, 0);
  TIZ_LOG (TIZ_PRIORITY_TRACE, "OMX_SetupTunnel [%s]", tiz_err_to_str(error));
  fail_if (OMX_ErrorNone != error);
  ...
  error = OMX_SendCommand (p_opusdec, OMX_CommandStateSet, OMX_StateIdle, NULL);
  TIZ_LOG (TIZ_PRIORITY_TRACE, "[%s] OMX_StateIdle [%s]",
             OPUS_DEC_COMPONENT_NAME, tiz_err_to_str(error));
  fail_if (OMX_ErrorNone != error);
  ...
  /* ----------------------------------------- */
  error = OMX_GetState (p_opusdec, &state);
  TIZ_LOG (TIZ_PRIORITY_TRACE, "[%s] state [%s]",
             OPUS_DEC_COMPONENT_NAME, tiz_state_to_str (state));
  fail_if (OMX_ErrorNone != error);
  fail_if (OMX_StateIdle != state);
  ...
  /* ------------------------------------ */
  error = _ctx_reset (&dec_ctx, OMX_EventCmdComplete);
  error = OMX_SendCommand (p_opusdec, OMX_CommandStateSet,
                           OMX_StateExecuting, NULL);
  fail_if (OMX_ErrorNone != error);
  ...
  /* Teardown tunnels */
  /* ---------------- */
  error = OMX_TeardownTunnel(p_oggdmux, 0, p_opusdec, 0);
  TIZ_LOG (TIZ_PRIORITY_TRACE, "OMX_TeardownTunnel [%s]", tiz_err_to_str(error));
  fail_if (OMX_ErrorNone != error);
  error = OMX_TeardownTunnel(p_opusdec, 1, p_pcmrnd, 0);
  TIZ_LOG (TIZ_PRIORITY_TRACE, "OMX_TeardownTunnel [%s]", tiz_err_to_str(error));
  fail_if (OMX_ErrorNone != error);
  ...
  fail_if (OMX_ErrorNone != error);
  error = OMX_FreeHandle (p_opusdec);
  fail_if (OMX_ErrorNone != error);
...
Suite * opusdec_suite (void)
{
  TCase *tc_md;
  Suite *s = suite_create ("libtizopusd");

  /* test case */
  tc_md = tcase_create ("OPUS Playback");
  tcase_add_unchecked_fixture (tc_md, setup, teardown);
  tcase_set_timeout (tc_md, OPUS_DECODER_TEST_TIMEOUT);
  tcase_add_loop_test (tc_md, test_opus_playback, 0, 1);
...
{
  int number_failed;
  SRunner *sr = srunner_create (opusdec_suite ());

  tiz_log_init();

  TIZ_LOG (TIZ_PRIORITY_TRACE, "Tizonia OpenMAX IL - Opus Decoder unit tests");
