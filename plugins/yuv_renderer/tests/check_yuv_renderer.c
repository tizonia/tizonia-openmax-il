...
#define TIZ_LOG_CATEGORY_NAME "tiz.yuv_renderer.check"
#endif

char *pg_rmd_path;
pid_t g_rmd_pid;

#define COMPONENT_NAME "OMX.Aratelia.iv_renderer.yuv.overlay"
                    ...
                    OMX_U32 nData1, OMX_U32 nData2, OMX_PTR pEventData)
{
  ...
  TIZ_LOG (TIZ_PRIORITY_TRACE, "Component Event [%d]", eEvent);
...
START_TEST (test_yuv_play)
{
  ...
  OMX_VIDEO_PARAM_PORTFORMATTYPE vidformat;
  ...
  /* ------------------------------------ */
  /* Obtain the video format from port #0 */
  /* ------------------------------------ */
  vidformat.nSize = sizeof (OMX_VIDEO_PARAM_PORTFORMATTYPE);
  vidformat.nVersion.nVersion = OMX_VERSION;
  vidformat.nPortIndex = 0;
  vidformat.nIndex = 0;
  error = OMX_GetParameter (p_hdl, OMX_IndexParamVideoPortFormat, &vidformat);
  TIZ_LOG (TIZ_PRIORITY_TRACE, "OMX_GetParameter (OMX_IndexParamVideoPortFormat) = [%s]",
  ...
  /* Total = 345600 */
  /* This is read-only */
  port_def.nBufferSize = 345600;
  port_def.format.video.nFrameWidth = 640;
  port_def.format.video.nFrameHeight = 360;
  port_def.format.video.nStride = 0;
  port_def.format.video.nSliceHeight = 0;
  port_def.format.video.nBitrate = 0;
  port_def.format.video.xFramerate = 0;
  port_def.format.video.bFlagErrorConcealment = OMX_FALSE;;
  ...
  TIZ_LOG (TIZ_PRIORITY_TRACE, "OMX_SetParameter (OMX_IndexParamPortDefinition)"
  ...
  vidformat.nPortIndex = 0;
  vidformat.nIndex = 0;
  vidformat.eCompressionFormat = OMX_VIDEO_CodingUnused;
  vidformat.eColorFormat = OMX_COLOR_FormatYUV420Planar;
  vidformat.xFramerate = 0;;

  error = OMX_SetParameter (p_hdl, OMX_IndexParamVideoPortFormat, &vidformat);
  TIZ_LOG (TIZ_PRIORITY_TRACE, "OMX_SetParameter (OMX_IndexParamVideoPortFormat)"
           " = [%s]", tiz_err_to_str (error));
  fail_if (OMX_ErrorNone != error);
  ...
  TIZ_LOG (TIZ_PRIORITY_TRACE, "after OMX_SendCommand");
...
ar_suite (void)
{
  ...
  TCase *tc_ivr;
  Suite *s = suite_create ("libtizsdlivr");
