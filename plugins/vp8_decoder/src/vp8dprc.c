...
static size_t
read_from_omx_buffer (void *ap_dst, const size_t a_bytes, OMX_BUFFERHEADERTYPE * ap_hdr)
{
...
get_output_buffer (vp8d_prc_t *ap_prc)
{
  ...
  if (NULL != ap_prc->p_outhdr_)
  ...
  else
    {
      ...
      TIZ_PD_ZERO (&ports);
      if (OMX_ErrorNone == tiz_krn_select (tiz_get_krn (handleOf (ap_prc)), 2, &ports))
        {
          if (TIZ_PD_ISSET (1, &ports))
            {
              if (OMX_ErrorNone == tiz_krn_claim_buffer
                  (tiz_get_krn (handleOf (ap_prc)), 1, 0, &ap_prc->p_outhdr_))
                {
                  TIZ_TRACE (handleOf (ap_prc), "Claimed output HEADER [%p]..."
                           "nFilledLen [%d]",
...
obtain_stream_info (vp8d_prc_t *ap_prc, OMX_BUFFERHEADERTYPE *p_inhdr)
{
  ...
  find_stream_info (p_inhdr->pBuffer, &(ap_prc->stream_type_),
                    &fourcc, &width, &height, &fps_den, &fps_num);

  TIZ_TRACE (handleOf (ap_prc), "Stream [%s] fourcc = [%d] width [%d] height [%d] "
            "fps_den [%d] fps_num [%d]",
            ap_prc->stream_type_ == STREAM_RAW ? "RAW" : "IVF",
            fourcc, width, height, fps_den, fps_num);
...
read_frame (vp8d_prc_t *ap_prc, OMX_BUFFERHEADERTYPE *p_inhdr)
{
  ...
  TIZ_TRACE (handleOf (ap_prc), "p_buf->frame_size [%d] p_buf->alloc_len [%d] "
  ...
  while (1)
    {
      ...
      if (!p_inhdr || !p_outhdr)
        {
          TIZ_TRACE (handleOf (ap_prc), "p_inhdr [%p] p_outhdr [%p]",
                    p_inhdr, p_outhdr);
...
vp8d_proc_allocate_resources (void *ap_obj, OMX_U32 a_pid)
{
  ...
  /* Initialize codec */
  if (VPX_CODEC_OK
      != (err = vpx_codec_dec_init (&(p_prc->vp8ctx_),
...
vp8d_prc_class_init (void * ap_tos, void * ap_hdl)
{
  ...
  void * vp8dprc_class = factory_new
    ...
    (classOf (tizprc), "vp8dprc_class", classOf (tizprc), sizeof (vp8d_prc_class_t),
     /* TIZ_CLASS_COMMENT: */
     ap_tos, ap_hdl,
