...
START_TEST (test_ar_play)
{
  ...
  TIZ_LOG (TIZ_TRACE, "OMX_GetParameter");
  ...
  TIZ_LOG (TIZ_TRACE, "nSamplingRate [%d]", _i);

  pcmmode.nSamplingRate = pg_rates[_i];

  TIZ_LOG (TIZ_TRACE, "after nSamplingRate [%d] [%p] [%p]",
             _i, p_hdl, &pcmmode);

  error = OMX_SetParameter (p_hdl, OMX_IndexParamAudioPcm, &pcmmode);
  TIZ_LOG (TIZ_TRACE, "OMX_SetParameter");
  fail_if (OMX_ErrorNone != error);
...
main (void)
{
  ...
  TIZ_LOG (TIZ_TRACE, "Tizonia OpenMAX IL - ALSA Audio Renderer unit tests");
