...
buffer_emptied (ar_prc_t * ap_prc)
{
  ...
  TIZ_TRACE (tiz_api_get_hdl (ap_prc), "HEADER [%p] emptied",
            ap_prc->p_inhdr_);
  ...
  if (NULL != p_obj->p_pcm_hdl)
    {
      ...
      TIZ_NOTICE (p_hdl, "nChannels = [%d] nBitPerSample = [%d] "
                ...
                p_obj->pcmmode.bInterleaved ==
                OMX_TRUE ? "OMX_TRUE" : "OMX_FALSE", p_obj->pcmmode.ePCMMode);
      ...
      snd_sampling_rate = p_obj->pcmmode.bInterleaved == OMX_TRUE ?
        p_obj->pcmmode.nSamplingRate : p_obj->pcmmode.nSamplingRate * 2;
      ...
      TIZ_DEBUG (p_hdl, "Poll descriptors : %d",
...
ar_prc_resume (const void *ap_obj)
{
  ...
  int resume = 0;
  assert (NULL != p_prc);
  TIZ_TRACE (tiz_api_get_hdl (p_prc), "RESUMING ALSA device...");
