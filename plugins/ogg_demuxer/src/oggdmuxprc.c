...
buffer_filled (oggdmux_prc_t * ap_prc, const OMX_U32 a_pid)
{
  ...
  TIZ_LOGN (TIZ_TRACE, tiz_api_get_hdl (ap_prc), "Releasing HEADER [%p] "
  ...
  (void) tiz_krn_release_buffer (tiz_get_krn (tiz_api_get_hdl (ap_prc)), a_pid, p_hdr);
  *pp_hdr = NULL;
}
             ...
             void *ap_user_data, const OMX_U32 a_pid)
{
  ...
  dump_into_omx_buffer (p_prc, a_pid, p_op, p_hdr);
...
release_buffers (oggdmux_prc_t * ap_prc, const OMX_U32 a_pid)
{
  ...
  TIZ_LOGN (TIZ_TRACE, tiz_api_get_hdl (ap_prc), "pid [%d]", a_pid);
                      ...
                      long a_serialno, void *ap_user_data)
{
  ...
  if (oggz_get_bos (ap_oggz, a_serialno) > 0)
    {
      TIZ_LOGN (TIZ_TRACE, tiz_api_get_hdl (p_prc), "serialno = [%d]", a_serialno);
      if (NULL == oggz_table_insert (p_prc->p_tracks_,
                                                               ...
                                                               * something */
        {
          TIZ_LOGN (TIZ_ERROR, tiz_api_get_hdl (p_prc),
                    "serialno = [%d] - Could not insert serialno in oggz table",
                    a_serialno);
          rc = OGGZ_STOP_ERR;
        }
    }

  if (oggz_get_bos (ap_oggz, -1) == 0)
    {
      TIZ_LOGN (TIZ_TRACE, tiz_api_get_hdl (p_prc),
                "Number of tracks [%d]",
...
set_read_page_callback (oggdmux_prc_t * ap_prc, OggzReadPage ap_read_cback)
{
  ...
  if (oggz_set_read_page (ap_prc->p_oggz_, -1, ap_read_cback, ap_prc) < 0)
    {
      TIZ_LOGN (TIZ_ERROR, tiz_api_get_hdl (ap_prc),
                "[OMX_ErrorInsufficientResources] : "
...
static inline OMX_ERRORTYPE
set_read_packet_callback (oggdmux_prc_t * ap_prc, OggzReadPacket ap_read_cback)
{
  assert (NULL != ap_prc);
  assert (NULL != ap_read_cback);
  if (oggz_set_read_callback (ap_prc->p_oggz_, -1, ap_read_cback, ap_prc) < 0)
    {
      TIZ_LOGN (TIZ_ERROR, tiz_api_get_hdl (ap_prc),
                "[OMX_ErrorInsufficientResources] : "
  ...
  if (NULL != ap_prc)
    {
      ...
      TIZ_LOGN (TIZ_TRACE, tiz_api_get_hdl (ap_prc), "%010lu: codec [%s]",
                serialno,
      ...
      if (is_audio_content (content))
        {
          ...
          tiz_check_omx_err
            (set_read_packet_callback (ap_prc, read_audio_packet));
        }
      ...
      if (is_video_content (content))
        {
          ...
          tiz_check_omx_err
            (set_read_packet_callback (ap_prc, read_video_packet));
        }
    }
...
demux_file (oggdmux_prc_t * ap_prc)
{
  ...
  TIZ_LOGN (TIZ_TRACE, tiz_api_get_hdl (ap_prc), "run_status [%d]",
            run_status);
...
oggdmux_prc_allocate_resources (void *ap_obj, OMX_U32 a_pid)
{
  ...
  TIZ_LOGN (TIZ_TRACE, tiz_api_get_hdl (p_prc), "Allocating resources");
...
oggdmux_prc_deallocate_resources (void *ap_obj)
{
  oggdmux_prc_t *p_prc = ap_obj;
  assert (NULL != p_prc);
  TIZ_LOGN (TIZ_TRACE, tiz_api_get_hdl (p_prc), "Deallocating resources");
  close_file (p_prc);
  delete_oggz (p_prc);
  delete_uri (p_prc);
...
oggdmux_prc_stop_and_return (void *ap_obj)
{
  oggdmux_prc_t *p_prc = ap_obj;
  assert (NULL != p_prc);
  TIZ_LOGN (TIZ_TRACE, tiz_api_get_hdl (p_prc), "stop_and_return");
