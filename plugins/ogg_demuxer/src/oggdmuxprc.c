...
obtain_uri (oggdmux_prc_t * ap_prc)
{
  ...
  TIZ_LOGN (TIZ_PRIORITY_NOTICE, tiz_api_get_hdl (ap_prc), "URI [%s]",
...
buffer_filled (oggdmux_prc_t * ap_prc, const OMX_U32 a_pid)
{
  ...
  TIZ_LOGN (TIZ_PRIORITY_TRACE, tiz_api_get_hdl (ap_prc), "Releasing HEADER [%p] "
...
release_buffers (oggdmux_prc_t * ap_prc, const OMX_U32 a_pid)
{
  ...
  TIZ_LOGN (TIZ_PRIORITY_TRACE, tiz_api_get_hdl (ap_prc), "pid [%d]", a_pid);
...
{
  assert (NULL != ap_prc);
  TIZ_LOGN (TIZ_PRIORITY_TRACE, tiz_api_get_hdl (ap_prc), "do_flush");
  ...
  if (oggz_get_bos (ap_oggz, a_serialno) > 0)
    {
      ...
      if (NULL == oggz_table_insert (p_prc->p_tracks_,
                                     ...
                                     &read_page_first_pass))  /* NULL makes it barf, needs
                                                               * something */
        {
          TIZ_LOGN (TIZ_PRIORITY_ERROR, tiz_api_get_hdl (p_prc), "serialno = [%d] - "
  ...
  if (NULL != ap_prc)
    {
      ...
      TIZ_LOGN (TIZ_PRIORITY_TRACE, tiz_api_get_hdl (ap_prc), "%010lu: codec [%s]",
                serialno,
...
demux_file (oggdmux_prc_t * ap_prc)
{
  ...
  TIZ_LOGN (TIZ_PRIORITY_TRACE, tiz_api_get_hdl (ap_prc), "run_status [%d]",
            run_status);
...
oggdmux_prc_allocate_resources (void *ap_obj, OMX_U32 a_pid)
{
  ...
  TIZ_LOGN (TIZ_PRIORITY_TRACE, tiz_api_get_hdl (p_prc), "Allocating resources");
  ...
  if ((p_prc->p_file_
    ...
    {
      TIZ_LOGN (TIZ_PRIORITY_ERROR, tiz_api_get_hdl (p_prc),
...
oggdmux_prc_deallocate_resources (void *ap_obj)
{
  oggdmux_prc_t *p_prc = ap_obj;
  assert (NULL != p_prc);
  TIZ_LOGN (TIZ_PRIORITY_TRACE, tiz_api_get_hdl (p_prc), "Deallocating resources");
  close_file (p_prc);
  delete_oggz (p_prc);
  delete_uri (p_prc);
...
oggdmux_prc_stop_and_return (void *ap_obj)
{
  oggdmux_prc_t *p_prc = ap_obj;
  assert (NULL != p_prc);
  TIZ_LOGN (TIZ_PRIORITY_TRACE, tiz_api_get_hdl (p_prc), "stop_and_return");
...
oggdmux_prc_port_disable (const void *ap_obj, OMX_U32 a_pid)
{
  ...
  *p_port_disabled = true;
  ...
  /* Release any buffers held  */
  TIZ_LOGN (TIZ_PRIORITY_TRACE, tiz_api_get_hdl (p_prc), "port_disable");
