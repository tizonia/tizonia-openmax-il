...
/**
 * @file   mp4dmuxsrcprc.c
 * @author Juan A. Rubio <juan.rubio@aratelia.com>
 *
 * @brief  Tizonia - MP4 Demuxer source processor
 *
 *
 */
...
#include "mp4dmuxsrcprc_decls.h"

#ifdef TIZ_LOG_CATEGORY_NAME
#undef TIZ_LOG_CATEGORY_NAME
#define TIZ_LOG_CATEGORY_NAME "tiz.mp4_demuxer.source.prc"
#endif

/* Forward declarations */
static OMX_ERRORTYPE
mp4dmuxsrc_prc_deallocate_resources (void *);
static OMX_ERRORTYPE
release_buffer (mp4dmuxsrc_prc_t *);
static OMX_ERRORTYPE
prepare_for_port_auto_detection (mp4dmuxsrc_prc_t * ap_prc);

#define on_mp4_error_ret_omx_oom(expr)                                   \
  do                                                                         \
    {                                                                        \
      int mp4_error = 0;                                                 \
      if (0 != (mp4_error = (expr)))                                     \
...
/* get_aud_hdr (mp4dmuxsrc_prc_t * ap_prc) */
/* { */
/*   assert (ap_prc); */
...
/* static inline OMX_BUFFERHEADERTYPE * */
/* get_vid_hdr (mp4dmuxsrc_prc_t * ap_prc) */
/* { */
/*   assert (ap_prc); */
/*   return tiz_filter_prc_get_header (ap_prc, */
...
/* static OMX_ERRORTYPE init_demuxer (mp4dmuxsrc_prc_t *ap_prc) */
/* { */
...
/* static OMX_ERRORTYPE transform_buffer (mp4dmuxsrc_prc_t *ap_prc) */
/* { */
/*   OMX_ERRORTYPE rc = OMX_ErrorNone; */
...
static void
update_cache_size (mp4dmuxsrc_prc_t * ap_prc)
{
  assert (ap_prc);
  assert (ap_prc->bitrate_ > 0);
  ap_prc->cache_bytes_ = ((ap_prc->bitrate_ * 1000) / 8)
                         * ARATELIA_MP4_DEMUXER_DEFAULT_CACHE_SECONDS;
  if (ap_prc->p_trans_)
    {
...
static OMX_ERRORTYPE
obtain_uri (mp4dmuxsrc_prc_t * ap_prc)
{
  OMX_ERRORTYPE rc = OMX_ErrorNone;
...
static void
obtain_audio_encoding_from_headers (mp4dmuxsrc_prc_t * ap_prc,
...
static inline void
delete_uri (mp4dmuxsrc_prc_t * ap_prc)
{
  assert (ap_prc);
...
static OMX_ERRORTYPE
release_buffer (mp4dmuxsrc_prc_t * ap_prc)
{
  assert (ap_prc);
  ...
  if (ap_prc->p_outhdr_)
    {
      ...
      tiz_check_omx (tiz_krn_release_buffer (
        ...
        ARATELIA_MP4_DEMUXER_SOURCE_PORT_0_INDEX, ap_prc->p_outhdr_));
      ...
      if (p_prc->p_outhdr_)
      ...
      else
        {
          if (OMX_ErrorNone == (tiz_krn_claim_buffer (
                                 ...
                                 ARATELIA_MP4_DEMUXER_SOURCE_PORT_0_INDEX, 0,
                                 &p_prc->p_outhdr_)))
            {
              if (p_prc->p_outhdr_)
                {
...
prepare_for_port_auto_detection (mp4dmuxsrc_prc_t * ap_prc)
{
  ...
  TIZ_INIT_OMX_PORT_STRUCT (port_def,
                            ARATELIA_MP4_DEMUXER_SOURCE_PORT_0_INDEX);
  tiz_check_omx (
...
static void *
mp4dmuxsrc_prc_ctor (void * ap_prc, va_list * app)
{
  mp4dmuxsrc_prc_t * p_prc
    = super_ctor (typeOf (ap_prc, "mp4dmuxsrcprc"), ap_prc, app);
  assert (p_prc);
  ...
  p_prc->bitrate_ = ARATELIA_MP4_DEMUXER_DEFAULT_BIT_RATE_KBITS;
  update_cache_size (p_prc);
  return p_prc;
}

static void *
mp4dmuxsrc_prc_dtor (void * ap_obj)
{
  (void) mp4dmuxsrc_prc_deallocate_resources (ap_obj);
  return super_dtor (typeOf (ap_obj, "mp4dmuxsrcprc"), ap_obj);
}

/* static OMX_ERRORTYPE */
/* mp4dmuxsrc_prc_read_buffer (const void *ap_obj, OMX_BUFFERHEADERTYPE * p_hdr)
 */
/* { */
/*   return OMX_ErrorNone; */
/* } */
...
static OMX_ERRORTYPE
mp4dmuxsrc_prc_allocate_resources (void * ap_prc, OMX_U32 a_pid)
{
  mp4dmuxsrc_prc_t * p_prc = ap_prc;
  OMX_ERRORTYPE rc = OMX_ErrorNone;
  assert (p_prc);
  assert (!p_prc->p_uri_);
...

  {
    ...
    rc
      = tiz_urltrans_init (&(p_prc->p_trans_), p_prc, p_prc->p_uri_,
                           ...
                           ARATELIA_MP4_DEMUXER_DEFAULT_RECONNECT_TIMEOUT,
...
static OMX_ERRORTYPE
mp4dmuxsrc_prc_deallocate_resources (void * ap_prc)
{
  mp4dmuxsrc_prc_t * p_prc = ap_prc;
  assert (p_prc);
...
static OMX_ERRORTYPE
mp4dmuxsrc_prc_prepare_to_transfer (void * ap_prc, OMX_U32 a_pid)
{
  mp4dmuxsrc_prc_t * p_prc = ap_prc;
  assert (ap_prc);
  p_prc->eos_ = false;
...
static OMX_ERRORTYPE
mp4dmuxsrc_prc_transfer_and_process (void * ap_prc, OMX_U32 a_pid)
{
  mp4dmuxsrc_prc_t * p_prc = ap_prc;
  OMX_ERRORTYPE rc = OMX_ErrorNone;
  assert (p_prc);
...
static OMX_ERRORTYPE
mp4dmuxsrc_prc_stop_and_return (void * ap_prc)
{
  mp4dmuxsrc_prc_t * p_prc = ap_prc;
  assert (p_prc);
  if (p_prc->p_trans_)
...
static OMX_ERRORTYPE
mp4dmuxsrc_prc_buffers_ready (const void * ap_prc)
{
  mp4dmuxsrc_prc_t * p_prc = (mp4dmuxsrc_prc_t *) ap_prc;
  assert (p_prc);
  return tiz_urltrans_on_buffers_ready (p_prc->p_trans_);
}

static OMX_ERRORTYPE
mp4dmuxsrc_prc_io_ready (void * ap_prc, tiz_event_io_t * ap_ev_io, int a_fd,
                          int a_events)
{
  mp4dmuxsrc_prc_t * p_prc = ap_prc;
  assert (p_prc);
...
static OMX_ERRORTYPE
mp4dmuxsrc_prc_timer_ready (void * ap_prc, tiz_event_timer_t * ap_ev_timer,
...
static OMX_ERRORTYPE
mp4dmuxsrc_prc_pause (const void * ap_obj)
{
  return OMX_ErrorNone;
}

static OMX_ERRORTYPE
mp4dmuxsrc_prc_resume (const void * ap_obj)
{
  return OMX_ErrorNone;
}

static OMX_ERRORTYPE
mp4dmuxsrc_prc_port_flush (const void * ap_obj, OMX_U32 TIZ_UNUSED (a_pid))
{
  mp4dmuxsrc_prc_t * p_prc = (mp4dmuxsrc_prc_t *) ap_obj;
  if (p_prc->p_trans_)
    {
      tiz_urltrans_flush_buffer (p_prc->p_trans_);
    }
...
static OMX_ERRORTYPE
mp4dmuxsrc_prc_port_disable (const void * ap_obj, OMX_U32 TIZ_UNUSED (a_pid))
{
  mp4dmuxsrc_prc_t * p_prc = (mp4dmuxsrc_prc_t *) ap_obj;
  assert (p_prc);
  ...
  return release_buffer ((mp4dmuxsrc_prc_t *) ap_obj);
}

static OMX_ERRORTYPE
mp4dmuxsrc_prc_port_enable (const void * ap_prc, OMX_U32 a_pid)
{
  mp4dmuxsrc_prc_t * p_prc = (mp4dmuxsrc_prc_t *) ap_prc;
  OMX_ERRORTYPE rc = OMX_ErrorNone;
  assert (p_prc);
...
/*
 * mp4dmuxsrc_prc_class
 */

static void *
mp4dmuxsrc_prc_class_ctor (void * ap_prc, va_list * app)
{
  /* NOTE: Class methods might be added in the future. None for now. */
  return super_ctor (typeOf (ap_prc, "mp4dmuxsrcprc_class"), ap_prc, app);
}
...
void *
mp4dmuxsrc_prc_class_init (void * ap_tos, void * ap_hdl)
{
  void * tizprc = tiz_get_type (ap_hdl, "tizprc");
  void * mp4dmuxsrcprc_class = factory_new
    /* TIZ_CLASS_COMMENT: class type, class name, parent, size */
    (classOf (tizprc), "mp4dmuxsrcprc_class", classOf (tizprc),
     sizeof (mp4dmuxsrc_prc_class_t),
     /* TIZ_CLASS_COMMENT: */
     ap_tos, ap_hdl,
     /* TIZ_CLASS_COMMENT: class constructor */
     ctor, mp4dmuxsrc_prc_class_ctor,
     /* TIZ_CLASS_COMMENT: stop value*/
     0);
  return mp4dmuxsrcprc_class;
}

void *
mp4dmuxsrc_prc_init (void * ap_tos, void * ap_hdl)
{
  void * tizprc = tiz_get_type (ap_hdl, "tizprc");
  void * mp4dmuxsrcprc_class = tiz_get_type (ap_hdl, "mp4dmuxsrcprc_class");
  TIZ_LOG_CLASS (mp4dmuxsrcprc_class);
  void * mp4dmuxsrcprc = factory_new
    /* TIZ_CLASS_COMMENT: class type, class name, parent, size */
    (mp4dmuxsrcprc_class, "mp4dmuxsrcprc", tizprc, sizeof (mp4dmuxsrc_prc_t),
     /* TIZ_CLASS_COMMENT: */
     ap_tos, ap_hdl,
     /* TIZ_CLASS_COMMENT: class constructor */
     ctor, mp4dmuxsrc_prc_ctor,
     /* TIZ_CLASS_COMMENT: class destructor */
     dtor, mp4dmuxsrc_prc_dtor,
     /* TIZ_CLASS_COMMENT: */
     tiz_srv_allocate_resources, mp4dmuxsrc_prc_allocate_resources,
     /* TIZ_CLASS_COMMENT: */
     tiz_srv_deallocate_resources, mp4dmuxsrc_prc_deallocate_resources,
     /* TIZ_CLASS_COMMENT: */
     tiz_srv_prepare_to_transfer, mp4dmuxsrc_prc_prepare_to_transfer,
     /* TIZ_CLASS_COMMENT: */
     tiz_srv_transfer_and_process, mp4dmuxsrc_prc_transfer_and_process,
     /* TIZ_CLASS_COMMENT: */
     tiz_srv_stop_and_return, mp4dmuxsrc_prc_stop_and_return,
     /* TIZ_CLASS_COMMENT: */
     tiz_srv_io_ready, mp4dmuxsrc_prc_io_ready,
     /* TIZ_CLASS_COMMENT: */
     tiz_srv_timer_ready, mp4dmuxsrc_prc_timer_ready,
     /* TIZ_CLASS_COMMENT: */
     tiz_prc_buffers_ready, mp4dmuxsrc_prc_buffers_ready,
     /* TIZ_CLASS_COMMENT: */
     tiz_prc_pause, mp4dmuxsrc_prc_pause,
     /* TIZ_CLASS_COMMENT: */
     tiz_prc_resume, mp4dmuxsrc_prc_resume,
     /* TIZ_CLASS_COMMENT: */
     tiz_prc_port_flush, mp4dmuxsrc_prc_port_flush,
     /* TIZ_CLASS_COMMENT: */
     tiz_prc_port_disable, mp4dmuxsrc_prc_port_disable,
     /* TIZ_CLASS_COMMENT: */
     tiz_prc_port_enable, mp4dmuxsrc_prc_port_enable,
     /* TIZ_CLASS_COMMENT: stop value */
     0);

  return mp4dmuxsrcprc;
}
