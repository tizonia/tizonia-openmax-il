...
set_audio_coding_on_port (chromecastrnd_prc_t * ap_prc)
{
  ...
  TIZ_INIT_OMX_PORT_STRUCT (port_def, ARATELIA_CHROMECAST_RENDERER_PORT_INDEX);
  tiz_check_omx (
...
set_mp3_audio_info_on_port (chromecastrnd_prc_t * ap_prc)
{
  ...
  TIZ_INIT_OMX_PORT_STRUCT (mp3type, ARATELIA_CHROMECAST_RENDERER_PORT_INDEX);
...
set_aac_audio_info_on_port (chromecastrnd_prc_t * ap_prc)
{
  ...
  TIZ_INIT_OMX_PORT_STRUCT (aactype, ARATELIA_CHROMECAST_RENDERER_PORT_INDEX);
  tiz_check_omx (tiz_api_GetParameter (tiz_get_krn (handleOf (ap_prc)),
                                       handleOf (ap_prc),
...
set_opus_audio_info_on_port (chromecastrnd_prc_t * ap_prc)
{
  ...
  TIZ_INIT_OMX_PORT_STRUCT (opustype, ARATELIA_CHROMECAST_RENDERER_PORT_INDEX);
  tiz_check_omx (
    tiz_api_GetParameter (tiz_get_krn (handleOf (ap_prc)), handleOf (ap_prc),
...
update_cache_size (chromecastrnd_prc_t * ap_prc)
{
  ...
  ap_prc->cache_bytes_ = ((ap_prc->bitrate_ * 1000) / 8)
                         * ARATELIA_CHROMECAST_RENDERER_DEFAULT_CACHE_SECONDS;
  if (ap_prc->p_trans_)
    {
  ...
  if (ap_prc->audio_coding_type_ != OMX_AUDIO_CodingUnused
      || ap_prc->audio_coding_type_ != OMX_AUDIO_CodingAutoDetect)
    {
      ...
      tiz_srv_issue_event ((OMX_PTR) ap_prc, OMX_EventPortSettingsChanged,
                           ARATELIA_CHROMECAST_RENDERER_PORT_INDEX, /* port 0 */
  ...
  if (p_prc->auto_detect_on_ && a_nbytes > 0)
    {
      ...
      /* This will pause the chromecast transfer */
      pause_needed = true;
...
chromecastrnd_prc_ctor (void * ap_obj, va_list * app)
{
  ...
  p_prc->auto_detect_on_ = false;
  p_prc->bitrate_ = ARATELIA_CHROMECAST_RENDERER_DEFAULT_BIT_RATE_KBITS;
...
chromecastrnd_prc_allocate_resources (void * ap_obj, OMX_U32 a_pid)
{
...

  {
    ...
    rc = tiz_urltrans_init (
      ...
      ARATELIA_CHROMECAST_RENDERER_DEFAULT_RECONNECT_TIMEOUT, buffer_cbacks,
