...
/**
 * @file   check_vorbis_decoder.c
 * @author Juan A. Rubio <juan.rubio@aratelia.com>
 *
 * @brief  Tizonia OpenMAX IL - Vorbis Decoder unit tests
 *
 *
 */
...
#define TIZ_LOG_CATEGORY_NAME "tiz.vorbis_decoder.check"
#endif
...
init_test_data()
{
  ...
  p_testfile1 = tiz_rcfile_get_value("plugins-data",
                                     "OMX.Aratelia.audio_decoder.vorbis.testfile1_uri");

  p_testfile2 = tiz_rcfile_get_value("plugins-data",
                                     "OMX.Aratelia.audio_decoder.vorbis.testfile2_uri");
...
START_TEST (test_vorbis_playback)
{
  ...
  /* --------------------------- */
  error = OMX_GetHandle (&p_vorbisdec, VORBIS_DEC_COMPONENT_NAME, (OMX_PTR *) (&dec_ctx),
                         &_check_cbacks);
  fail_if (OMX_ErrorNone != error);
  ...
  error = OMX_GetParameter (p_vorbisdec, OMX_IndexParamPortDefinition, &dec_port_def0);
  fail_if (OMX_ErrorNone != error);
  ...
  error = OMX_GetParameter (p_vorbisdec, OMX_IndexParamPortDefinition, &dec_port_def1);
  fail_if (OMX_ErrorNone != error);

  TIZ_LOG (TIZ_PRIORITY_TRACE, "[%s] port #1 nBufferSize [%d]",
             VORBIS_DEC_COMPONENT_NAME, dec_port_def1.nBufferSize);
  TIZ_LOG (TIZ_PRIORITY_TRACE, "[%s] port #1 nBufferCountActual [%d]",
             VORBIS_DEC_COMPONENT_NAME, dec_port_def1.nBufferCountActual);
  ...
  /* -------------------------------------------------- */
  /* Obtain the vorbis settings from the decoder's port #0 */
  /* -------------------------------------------------- */
  dec_vorbis_type.nSize = sizeof (OMX_AUDIO_PARAM_VORBISTYPE);
  dec_vorbis_type.nVersion.nVersion = OMX_VERSION;
  dec_vorbis_type.nPortIndex = 0;
  error = OMX_GetParameter (p_vorbisdec, OMX_IndexParamAudioVorbis, &dec_vorbis_type);
  fail_if (OMX_ErrorNone != error);
  ...
  error = OMX_GetParameter (p_vorbisdec, OMX_IndexParamAudioPcm, &dec_pcm_mode);
  fail_if (OMX_ErrorNone != error);
  ...
  error = OMX_SetParameter (p_vorbisdec, OMX_IndexParamAudioPcm, &dec_pcm_mode);
  ...
  /* ----------------------------------------- */
  error = OMX_GetState (p_vorbisdec, &state);
  TIZ_LOG (TIZ_PRIORITY_TRACE, "[%s] state [%s]",
             VORBIS_DEC_COMPONENT_NAME, tiz_state_to_str (state));
  fail_if (OMX_ErrorNone != error);
  fail_if (OMX_StateIdle != state);
  ...
  /* ------------------------------------ */
  error = _ctx_reset (&dec_ctx, OMX_EventCmdComplete);
  error = OMX_SendCommand (p_vorbisdec, OMX_CommandStateSet,
...
Suite * vorbisdec_suite (void)
{
  TCase *tc_md;
  Suite *s = suite_create ("libtizvorbisd");

  /* test case */
  tc_md = tcase_create ("VORBIS Playback");
  tcase_add_unchecked_fixture (tc_md, setup, teardown);
  tcase_set_timeout (tc_md, VORBIS_DECODER_TEST_TIMEOUT);
  tcase_add_loop_test (tc_md, test_vorbis_playback, 0, 1);
...
{
  int number_failed;
  SRunner *sr = srunner_create (vorbisdec_suite ());

  tiz_log_init();

  TIZ_LOG (TIZ_PRIORITY_TRACE, "Tizonia OpenMAX IL - Vorbis Decoder unit tests");
