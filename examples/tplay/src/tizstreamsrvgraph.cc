...
tizstreamsrvgraph::configure_stream ()
{
  ...
  rc = OMX_SetParameter (handles_[0], OMX_IndexParamContentURI, p_uritype);
  ...
  tiz_check_omx_err (
      OMX_GetParameter (handles_[1], OMX_IndexParamAudioMp3, &mp3type));
...
void tizstreamsrvgraph::do_unload ()
{
...
void tizstreamsrvgraph::do_error (const OMX_ERRORTYPE error)
{
...
void tizstreamsrvgraph::do_eos (const OMX_HANDLETYPE handle)
{
