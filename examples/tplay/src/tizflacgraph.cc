...
tizflacgraph::tizflacgraph (tizprobe_ptr_t probe_ptr)
  : tizgraph ("tizflacgraph", 3, probe_ptr),
    demuxer_index_ (0),
    demux_attempts_ (0)
{
}
...
tizflacgraph::disable_demuxer_video_port ()
{
  ...
  if (error == OMX_ErrorNone)
  {
    ...
    event_list.push_back (omx_event_info (handles_[0], OMX_EventCmdComplete,
                                          cmd, port_id,
...
tizflacgraph::configure_flac_graph (const int file_index)
{
  ...
  p_uritype->nSize = sizeof(OMX_PARAM_CONTENTURITYPE) + OMX_MAX_STRINGNAME_SIZE;
  ...
  tiz_check_omx_err (OMX_GetParameter (
      handles_[1], static_cast<OMX_INDEXTYPE>(OMX_TizoniaIndexParamAudioFlac),
      &flactype));
      ...
      || flactype_orig.nChannels != flactype.nChannels)
  {
    ...
    waitevent_list_t event_list (
        1,
        omx_event_info (handles_[1], OMX_EventPortSettingsChanged, 1,  // nData1
  ...
  tiz_check_omx_err (
      OMX_SetParameter (handles_[2], OMX_IndexParamAudioPcm, &pcmtype));
  return OMX_ErrorNone;
}

void tizflacgraph::unload_flac_graph ()
{
...
tizflacgraph::do_execute ()
{
  TIZ_LOG (TIZ_PRIORITY_TRACE,
           "current_file_index_ [%d] list size [%d]..."
           " demux_attempts_ [%d]",
...
tizflacgraph::do_skip (const int jump)
{
  ...
  if (OMX_StateExecuting != current_graph_state_ && OMX_StatePause
...
void tizflacgraph::do_error (const OMX_ERRORTYPE error)
{
  TIZ_LOG (TIZ_PRIORITY_TRACE,
           "[%s] - demux_attempts_ [%d]..."
           " current_file_index_ [%d]",
           tiz_err_to_str (error), demux_attempts_, current_file_index_);

  if ((error == OMX_ErrorFormatNotDetected || error == OMX_ErrorStreamCorrupt)
      && demux_attempts_ < kMaxAvailableDemuxers)
  {
    ...
    demuxer_index_
        = (demuxer_index_ + 1 == kMaxAvailableDemuxers ? 0 : demuxer_index_
                                                             + 1);
    (void)do_load ();
...
void tizflacgraph::do_eos (const OMX_HANDLETYPE handle)
{
  TIZ_LOG (TIZ_PRIORITY_TRACE,
           "demux_attempts_ [%d]..."
           " current_file_index_ [%d]",
