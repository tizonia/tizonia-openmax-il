...
"""@package tizpandoraproxy
Simple Pandora API proxy/wrapper.

Access Pandora to retrieve audio track URLs and create a playback queue.
...
if os.environ.get('TIZONIA_PANDORAPROXY_DEBUG'):
...
def exception_handler(exception_type, exception, traceback):
    ...
    print_err("[Pandora] (%s) : %s" % (exception_type.__name__, exception))
...
class TrackInfo(object):
    """ Class that represents a Pandora track in the queue.
...
class tizpandoraproxy(object):
    """A class that accesses Pandora servers, retrieves track URLs and creates and
    ...
    def __init__(self, base_url, token):
        ...
        self._pandora = PandoraServer(base_url, token)
        self._music = self._pandora.library.section('Music')
    ...
    def enqueue_audio_tracks(self, arg):
        """Search the Pandora server for audio tracks and add them to the playback queue.

        :param arg: a search string

        """
        logging.info('arg : %s', arg)
        print_msg("[Pandora] [Track search in server] : '{0}'. " \
    ...
    def enqueue_audio_artist(self, arg):
        """Obtain an artist from the Pandora server and add all the artist's audio tracks
        ...
        """
        logging.info('arg : %s', arg)
        print_msg("[Pandora] [Artist search in server] : '{0}'. " \
                  .format(self.base_url))
        try:
            count = len(self.queue)
            artist = None
    ...
    def enqueue_audio_album(self, arg):
        """Obtain an album from the Pandora server and add all its tracks to the playback
        queue.
        ...
        """
        logging.info('arg : %s', arg)
        print_msg("[Pandora] [Album search in server] : '{0}'. " \
                  .format(self.base_url))
        try:
            count = len(self.queue)
            album = None
    ...
    def enqueue_audio_playlist(self, arg):
        """Add all audio tracks in a Pandora playlist to the playback queue.

        :param arg: a playlist search term

        """
        logging.info('arg : %s', arg)
        print_msg("[Pandora] [Playlist search in server] : '{0}'. " \
                  .format(self.base_url))
        try:
            count = len(self.queue)
            playlist_title = ''
            ...
            try:
                playlist = self._pandora.playlist(title=arg)
                if playlist:
                    ...
                    for item in playlist.items():
                        if item.TYPE == 'track':
                            ...
                            self.add_to_playback_queue(track_info)
                        if count == len(self.queue):
                            print_wrn("[Pandora] '{0}' No audio tracks found." \
                                      .format(playlist_title.encode('utf-8')))
                            raise ValueError
            ...
            if count == len(self.queue):
                ...
                playlists = self._pandora.playlists()
                for pl in playlists:
    ...
    def remove_current_url(self):
        ...
        if len(self.queue) and self.queue_index:
            ...
            print_nfo("[Pandora] [Track] '{0}' removed." \
    ...
    def __update_play_queue_order(self):
        ...
        if total_tracks:
            ...
            print_nfo("[Pandora] [Tracks in queue] '{0}'." \
                      .format(total_tracks))
