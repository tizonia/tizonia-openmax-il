...
#include "services/deezer/tizdeezerconfig.hpp"
#include "services/deezer/tizdeezermgr.hpp"
...
void tiz::playapp::set_option_handlers ()
{
  ...
  // Deezer audio streaming client program options
  popts_.set_option_handler ("deezer-stream",
                             boost::bind (&tiz::playapp::deezer_stream, this));
}
...
tiz::playapp::deezer_stream ()
{
  ...
  // Instantiate the streaming client manager
  tiz::graphmgr::mgr_ptr_t p_mgr
      = boost::make_shared< tiz::graphmgr::deezermgr >(config);
