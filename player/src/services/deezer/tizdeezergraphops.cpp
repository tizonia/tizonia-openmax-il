...
/**
 * @file   tizdeezergraphops.cpp
 * @author Juan A. Rubio <juan.rubio@aratelia.com>
 *
 * @brief  Deezer client graph actions / operations implementation
 *
 */
...
#include "tizdeezerconfig.hpp"
#include "tizdeezergraphops.hpp"

#ifdef TIZ_LOG_CATEGORY_NAME
#undef TIZ_LOG_CATEGORY_NAME
#define TIZ_LOG_CATEGORY_NAME "tiz.play.graph.deezer.ops"
#endif
...
// deezerops
//
graph::deezerops::deezerops (graph *p_graph,
...
void graph::deezerops::do_enable_auto_detection (const int handle_id,
                                                 const int port_id)
{
  tizdeezerconfig_ptr_t deezer_config
...
{
  OMX_U32 deezer_source_port = port_id;
  G_OPS_BAIL_IF_ERROR (util::disable_port (handles_[comp_id], deezer_source_port),
                       "Unable to disable deezer source's output port.");
  clear_expected_port_transitions ();
  add_expected_port_transition (handles_[comp_id], deezer_source_port,
                                OMX_CommandPortDisable);
}

void graph::deezerops::do_configure_comp (const int comp_id)
{
  if (comp_id == 0)
  {
    tizdeezerconfig_ptr_t deezer_config
    ...
    G_OPS_BAIL_IF_ERROR (
        set_deezer_playlist (handles_[0], playlist_->get_current_uri ()),
        "Unable to set OMX_TizoniaIndexParamAudioDeezerPlaylist");
  }
}

void graph::deezerops::do_load ()
{
  assert (!comp_lst_.empty ());
  assert (!role_lst_.empty ());

  // At this point we are going to instantiate the remaining component in the
  // graph, the audio decoder and the pcm renderer. The deezer source is already
  // instantiated and in
  // Executing state.
  ...
  G_OPS_BAIL_IF_ERROR (
      get_encoding_type_from_deezer_source (),
      "Unable to retrieve the audio encoding from the deezer source.");
...
void graph::deezerops::do_configure ()
{
...
void graph::deezerops::do_loaded2idle ()
{
  if (last_op_succeeded ())
  {
...
void graph::deezerops::do_idle2exe ()
{
  if (last_op_succeeded ())
  {
...
void graph::deezerops::do_reconfigure_tunnel (const int tunnel_id)
{
  if (last_op_succeeded ())
  {
    if (0 == tunnel_id)
...
void graph::deezerops::do_retrieve_metadata ()
{
  OMX_U32 index = 0;
  const int deezer_index = 0;
  // Extract metadata from the deezer source
  while (OMX_ErrorNone == dump_metadata_item (index++, deezer_index))
  {
  };
    ...
    const int tunnel_id, const OMX_COMMANDTYPE to_disabled_or_enabled)
{
  ...
  {
    const int deezer_source_index = 0;
    const int deezer_source_output_port = 0;
    add_expected_port_transition (handles_[deezer_source_index],
                                  deezer_source_output_port,
...
bool graph::deezerops::probe_stream_hook ()
{
  return true;
}

OMX_ERRORTYPE graph::deezerops::get_encoding_type_from_deezer_source ()
{
...
OMX_ERRORTYPE
graph::deezerops::override_decoder_and_renderer_sampling_rates ()
{
  OMX_U32 channels = 2;
...
OMX_ERRORTYPE
graph::deezerops::apply_pcm_codec_info_from_decoder ()
{
  OMX_U32 channels = 2;
  OMX_U32 sampling_rate = 44100;
...
OMX_ERRORTYPE
graph::deezerops::get_channels_and_rate_from_decoder (
...
OMX_ERRORTYPE
graph::deezerops::set_channels_and_rate_on_decoder (
...
OMX_ERRORTYPE
graph::deezerops::set_channels_and_rate_on_renderer (
    const OMX_U32 channels, const OMX_U32 sampling_rate)
{
...
OMX_ERRORTYPE
graph::deezerops::set_deezer_playlist (const OMX_HANDLETYPE handle,
                                       const std::string &playlist)
{
  // Set the Deezer playlist
  OMX_TIZONIA_AUDIO_PARAM_DEEZERPLAYLISTTYPE playlisttype;
  TIZ_INIT_OMX_STRUCT (playlisttype);
  tiz_check_omx (OMX_GetParameter (
      handle,
      static_cast< OMX_INDEXTYPE >(OMX_TizoniaIndexParamAudioDeezerPlaylist),
      &playlisttype));
...
bool graph::deezerops::is_fatal_error (const OMX_ERRORTYPE error) const
{
  bool rc = false;
...
void graph::deezerops::do_record_fatal_error (const OMX_HANDLETYPE handle,
...
void graph::deezerops::do_reconfigure_first_tunnel ()
{
  // Retrieve the mp3 settings from the deezer source component
  OMX_AUDIO_PARAM_MP3TYPE deezer_mp3type;
  const OMX_U32 deezer_port_id = 0;
  TIZ_INIT_OMX_PORT_STRUCT (deezer_mp3type, deezer_port_id);
  G_OPS_BAIL_IF_ERROR (
      OMX_GetParameter (handles_[0], OMX_IndexParamAudioMp3, &deezer_mp3type),
      "Unable to retrieve the MP3 settings from the deezer source");
  ...
  decoder_mp3type.nChannels = deezer_mp3type.nChannels;
  decoder_mp3type.nSampleRate = deezer_mp3type.nSampleRate;
...
void graph::deezerops::do_reconfigure_second_tunnel ()
{
