...
/**
 * @file   tizdeezermgr.cpp
 * @author Juan A. Rubio <juan.rubio@aratelia.com>
 *
 * @brief  Manager implementation for the Deezer client graph
 *
 */
...
#include "tizdeezermgr.hpp"

#ifdef TIZ_LOG_CATEGORY_NAME
#undef TIZ_LOG_CATEGORY_NAME
#define TIZ_LOG_CATEGORY_NAME "tiz.play.deezermgr"
#endif
...
// mgr
//
graphmgr::deezermgr::deezermgr (tizgraphconfig_ptr_t config)
  : graphmgr::mgr (), config_ (config)
{
  TIZ_LOG (TIZ_PRIORITY_TRACE, "Constructing...");
}

graphmgr::deezermgr::~deezermgr ()
{
}

graphmgr::ops *graphmgr::deezermgr::do_init (
    ...
    graphmgr_capabilities_t &graphmgr_caps)
{
  ...
  return new deezermgrops (this, playlist, termination_cback);
}

//
// decodemgrops
//
graphmgr::deezermgrops::deezermgrops (
...
tizgraph_ptr_t graphmgr::deezermgrops::get_graph (
    const std::string & /* uri */)
{
  ...
  {
    g_ptr = boost::make_shared< tiz::graph::deezer >();
    if (g_ptr)
    {
      // TODO: Check rc
    ...
    {
      GMGR_OPS_RECORD_ERROR (OMX_ErrorInsufficientResources,
                             "Unable to create the Deezer client graph.");
    }
  }
  else
  {
...
void graphmgr::deezermgrops::do_load ()
{
...
void graphmgr::deezermgrops::do_execute ()
{
  assert (playlist_);
  assert (p_mgr_);

  deezermgr *p_clientmgr = dynamic_cast< deezermgr * >(p_mgr_);
  assert (p_clientmgr);
...
bool graphmgr::deezermgrops::is_fatal_error (const OMX_ERRORTYPE error,
