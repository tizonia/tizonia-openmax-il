...
/**
 * @file   tizdirblemgr.cpp
 * @author Juan A. Rubio <juan.rubio@aratelia.com>
 *
 * @brief  Manager implementation for the Dirble client graph
 *
 */
...
#include "tizdirblemgr.hpp"

#ifdef TIZ_LOG_CATEGORY_NAME
#undef TIZ_LOG_CATEGORY_NAME
#define TIZ_LOG_CATEGORY_NAME "tiz.play.dirblemgr"
#endif
...
// mgr
//
graphmgr::dirblemgr::dirblemgr (tizgraphconfig_ptr_t config)
  : graphmgr::mgr (), config_ (config)
{
  TIZ_LOG (TIZ_PRIORITY_TRACE, "Constructing...");
}

graphmgr::dirblemgr::~dirblemgr ()
{
}

graphmgr::ops *graphmgr::dirblemgr::do_init (
    ...
    graphmgr_capabilities_t &graphmgr_caps)
{
  ...
  graphmgr_caps.uri_schemes_ = boost::assign::list_of ("dirble");
  ...
  return new dirblemgrops (this, playlist, termination_cback);
}

//
// decodemgrops
//
graphmgr::dirblemgrops::dirblemgrops (
...
void graphmgr::dirblemgrops::do_execute ()
{
  assert (playlist_);
  assert (p_mgr_);

  dirblemgr *p_clientmgr = dynamic_cast< dirblemgr * >(p_mgr_);
  assert (p_clientmgr);
